...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, isCorrect, colorData }) => {
  ...
  return (
    ...
    >
      <div 
        className={`relative w-full max-w-md rounded-2xl shadow-2xl bg-white text-center transform transition-all duration-300 ease-out scale-95 animate-fade-in-up border-t-8 ${borderColor}`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 sm:p-8">
            <div className={`mx-auto w-20 h-20 flex items-center justify-center rounded-full mb-4 ${iconBgColor} ${iconColor}`}>
              {icon}
            </div>
            ...
            <div className="my-6 p-4 rounded-lg bg-slate-50 text-left">
                ...
                <div className="mt-4 border-t border-slate-200 pt-3 text-sm text-slate-600">
                    <div className="space-y-1">
                        <p><span className="font-semibold text-slate-700">マンセル値:</span> {munsell_value}</p>
                        <p><span className="font-semibold text-slate-700">PCCS:</span> {pccs_notation}</p>
                    </div>
                </div>
            </div>

            <p className="text-slate-600 text-left text-base leading-relaxed mb-8">
                {explanation}
            </p>
