...
const QuizScreen: React.FC<QuizScreenProps> = ({ grade, onQuizComplete, onGoHome }) => {
  ...
  return (
    <>
      <div className="w-full max-w-2xl mx-auto p-4 sm:p-6 flex flex-col h-full">
        ...
        <main className="flex-grow flex flex-col items-center justify-center">
          <div className="w-full max-w-md bg-white p-6 rounded-2xl shadow-2xl border border-slate-200">
             <div
               className="w-full h-48 sm:h-64 rounded-xl mb-8 border-8 border-slate-100 shadow-inner"
               style={{ backgroundColor: currentQuestion.correctAnswer.color_code }}
             ></div>
               ...
               {currentQuestion.options.map((option, index) => {
                  ...
                  if (isSelected) {
                     buttonStyle = isCorrect
                        ? "bg-green-500 text-white border-green-600 ring-4 ring-green-500/30"
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-lg text-lg text-center font-bold border-2 transition-all duration-300 transform ${buttonStyle} ${!selectedAnswer ? 'active:scale-95' : 'cursor-not-allowed'}`}
                    >
                      {option}
                    </button>
                  );
      ...
      {isModalOpen && (
         <Modal
            ...
            isCorrect={isCorrect}
            colorData={currentQuestion.correctAnswer}
        />
      )}
    </>
  );
